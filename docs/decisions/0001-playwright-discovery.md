...
sibling temporary file and rename. Source-test diagnostics use
configuration-relative POSIX paths when a config file is present.
...
rejected as ambiguous. A real project literally named `<default>` is rejected:
inventory consumers cannot distinguish that name from the implicit project, so
the reporter reserves the sentinel for the implicit project.
