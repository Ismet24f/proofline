...
`working-directory` must be repository-relative; every other action path is relative to that directory. Proofline rejects absolute paths and `..` traversal. The working directory is resolved against `GITHUB_WORKSPACE` and rejected if it is not a directory or escapes through a symlink. Existing path inputs, including an explicitly supplied Playwright config, are then resolved before use and rejected when their resolved target escapes that working directory. Artifact discovery does not follow symlinks. These checks are performed at access time; v0.1 does not claim protection against another process concurrently replacing validated filesystem entries.
