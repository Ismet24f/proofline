...
Proofline v0.1 is an unreleased open-source, local-only GitHub Action with three operations: `plan`, `collect`, and `reconcile`. The action and its committed `check/dist` bundle are implemented, independently reviewed, merged, and now in Phase C dogfood. Do not depend on `v0.1.0` until the dogfood gate passes and that tag is published.
...
The [roadmap and release checklist](docs/roadmap.md) keep technical readiness separate from demand. Phase C uses a machine-checked [dogfood protocol](docs/validation/phase-c-dogfood.md); external validation uses one frozen [30-day decision gate](docs/validation/decision-gate.md), a freeze manifest, and normalized machine-checked alias-only ledgers. No pilot clock has started, no external adoption is claimed, and the current market verdict is **promising, not proven**.
